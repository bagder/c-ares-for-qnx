...
for(@files) {
    my $file = $_;
    if($file =~ /^c-ares-([0-9.-]+)-qnxsdp(\S+)\.(tar\.gz)$/) {
...
for my $version (reverse sort { num($a) <=> num($b) } keys %versions) {
    ...
    my $link = $officialver;
    $link =~ s/\./_/g;
    print "#define QNX_CARESVER $officialver\n";
    print "#define QNX_CARESVER_PACKAGE $version\n";
    print "#define QNX_CARESVER_LINK c-ares-$link\n\n";
